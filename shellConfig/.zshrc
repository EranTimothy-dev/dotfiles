...
# expor PATH=$HOME/bin:$HOME/.local/bin:/usr/local/bin:$PATH
# Ensure function definitions are available
...
function python_venv_prompt() {
  if [[ "$OMB_PROMPT_SHOW_PYTHON_VENV" == "true" ]]; then
...
OMB_PROMPT_SHOW_PYTHON_VENV=true  # enable venv display
color_prompt=yes  # enable color prompt if needed
...
if [ "$color_prompt" = yes ]; then
    ...
    local venv_part=$(python_venv_prompt)
    #echo "env is: $venv_part"

    PROMPT=$'%F{%(#.blue.green)}┌──'"%B%F{reset}${venv_part}%b%F{%(#.blue.green)}"$'${debian_chroot:+($debian_chroot)──}(%B%F{%(#.red.blue)}%n%(#.💀.㉿)%m%b%F{%(#.blue.green)})-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.green)}]'"%B%F{reset}$(parse_git_branch)%b%F{%(#.blue.green)}"$'\n└─%B%(#.%F{red}#.%F{blue}$)%b%F{reset} '

    #PROMPT="%F{%(#.blue.green)}┌──[${venv_part}]${chroot_part}(%B%F{%(#.red.blue)}%n%(#.💀.㉿)%m%b%F{%(#.blue.green)})-[%B%F{reset}%~%b%F{%(#.blue.green)}]\n└─%B%(#.%F{red}#.%F{blue}$)%b%f "
